...
def curate_ques(state: AgentSchema) -> AgentSchema: 
    ...
    response = llm.invoke(f"Curate the following question: {user_question}").content[0]["text"]
...
def generate_sql(state: AgentSchema) -> AgentSchema:
    ...
    generated_sql_query = llm.invoke(prompt).content[0]["text"]  # Generate the SQL query using the LLM

    state.generated_sql_query = str(generated_sql_query)
